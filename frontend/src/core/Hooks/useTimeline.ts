...
export const useTimeline = ({
  ...
  const [relativeProgression, setRelativeProgression] = useState<
    number | undefined
  >(undefined);
  const [totalProgression, setTotalProgression] = useState<number | undefined>(
    undefined,
  );
  ...
  const timeline = useMemo(
    () => ({
      ...
      progression: {
        ...
        currentIndex: currentItem?.readingOrderIndex
          ? currentItem.readingOrderIndex + 1
          : undefined,
        ...
        positionsLeft:
          currentItem?.positionRange?.[1] && currentPositions[0] !== undefined
      ...
      return links.map((link) => {
        ...
        if (fragmentIndex !== -1) {
          ...
          const duplicateLink = links.find(
            (l) => l.href.startsWith(baseHref) && l.href !== href,
          );
          if (!duplicateLink) {
            href = baseHref;
          }
        ...
        const treeNode: TocItem = {
          ...
          href: href,
          title: link.title,
          position: positionsList?.find((position) => position.href === href)
            ?.locations.position,
        };

        // Recursively process children if they exist
        if (link.children) {
          treeNode.children = buildTocTree(
            link.children.items,
            idGenerator,
            positionsList,
          );
  ...
  const handleTocEntryOnNav = useCallback(
    (locator?: Locator) => {
      ...
      if (layout === Layout.fixed) {
        ...
        if (positions && positions.length === 2) {
          const otherPosition =
            positions[0] === locator.locations.position
              ? positions[1]
              : positions[0];
          const otherPositionInList = positionsList.find(
            (pos: Locator) => pos.locations.position === otherPosition,
          );
      ...
      const otherPositions = currentPositions.filter(
        (pos) => pos !== locator.locations.position,
      );
  ...
  const buildTimelineItems = useCallback(() => {
    ...
    const findNearestTitle = (currentHref: string): string => {
      const currentIndex = readingOrder.findIndex(
        (item) => getBaseUrl(item.href) === getBaseUrl(currentHref),
      );
      ...
      for (let i = currentIndex; i >= 0; i--) {
        ...
        const matchingTocItems = flatToc.filter(
          (t) => getBaseUrl(t.href) === getBaseUrl(item.href),
        );
    ...
    readingOrder.forEach((item, index) => {
      ...
      const timelineItem: TimelineItem = {
        ...
        title:
          item.title ||
          matchingTocItems[0]?.title ||
          findNearestTitle(item.href),
        fragments: matchingTocItems
          .map((t) => t.href.split("#")[1])
          .filter(Boolean),
        children:
          matchingTocItems[0]?.children?.items?.map((child) => ({
    ...
    for (const item of readingOrder) {
      ...
      const positions = positionsList
        ...
        .sort(
          (a, b) => (a.locations.position || 0) - (b.locations.position || 0),
        );
      ...
      if (positions.length > 0) {
        ...
        timelineItem.positionRange =
          start.position !== undefined
            ? [start.position, end?.position]
            : undefined;

        timelineItem.progressionRange =
          start.progression !== undefined
            ? [start.progression, end?.progression]
            : undefined;

        timelineItem.totalProgressionRange =
          start.totalProgression !== undefined
  ...
  const updateTimelineItems = useCallback(() => {
    ...
    const currentIndex = timelineItemsArray.findIndex(
      (item) => item.href === currentLocation.href,
    );
    ...
    if (currentIndex === -1) {
      ...
      setPreviousItem(
        currentIndex > 0 ? timelineItemsArray[currentIndex - 1] : null,
      );
      setNextItem(
        currentIndex < timelineItemsArray.length - 1
          ? timelineItemsArray[currentIndex + 1]
          : null,
      );
    }
  ...
  useEffect(() => {
    ...
    setTocTree(
      buildTocTree(publication.toc?.items || [], idGenerator, positionsList),
    );
