  ...
  reducers: {
      ...
      switch (action.payload.dockingKey) {
        case ThDockingKeys.start:
          ...
          for (const key in state.keys) {
            if (
              state.keys[key as ActionsStateKeys]?.docking ===
              action.payload.dockingKey
            ) {
        ...
        case ThDockingKeys.end:
          ...
          // And remove it from the other one
          if (
            state.dock[ThDockingKeys.start].actionKey === action.payload.key
          ) {
            state.dock[ThDockingKeys.start] = {
              ...state.dock[ThDockingKeys.start],
              actionKey: null,
    ...
    toggleActionOpen: (state, action: ActionStateTogglePayload) => {
      const payload = {
        ...
        isOpen: state.keys[action.payload.key]?.isOpen
          ? !state.keys[action.payload.key]?.isOpen
          : true,
      };
    ...
    setDockPanelWidth: (state, action: ActionStateSlotWidthPayload) => {
      ...
      const key: ActionsStateKeys | null =
        state.dock[action.payload.key].actionKey;
      if (key) {
        state.keys[key] = {
          ...state.keys[key],
