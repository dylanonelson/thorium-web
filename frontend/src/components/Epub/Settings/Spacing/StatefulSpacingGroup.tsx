...
export const StatefulSpacingGroupContainer = () => {
  ...
  const displayOrder =
    (preferences.settings.spacing?.subPanel as
      | ThSpacingSettingsKeys[]
      | null
      | undefined) || defaultSpacingSettingsSubpanel;
  const { spacingSettingsComponentsMap } = usePlugins();
  ...
  return (
    <>
      {displayOrder.map((key: ThSpacingSettingsKeys) => {
        ...
        if (!match) {
          console.warn(
            `Setting key "${key}" not found in the plugin registry while present in preferences.`,
          );
          return null;
        }
