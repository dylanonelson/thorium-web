...
export const StatefulParagraphIndent = ({
  standalone = true,
  ...
  const paragraphIndentRangeConfig = {
    ...
    placeholder:
      preferences.settings.keys[ThSettingsKeys.paragraphIndent].placeholder,
  ...
  const placeholderText = usePlaceholder(
    paragraphIndentRangeConfig.placeholder,
    paragraphIndentRangeConfig.range,
    "multiplier",
  );

  const { getSetting, submitPreferences } = useEpubNavigator();

  const { getEffectiveSpacingValue, setParagraphIndent, canBeReset } =
    useSpacingPresets();
  ...
  return (
    <>
      {paragraphIndentRangeConfig.variant ===
      ThSettingsRangeVariant.numberField ? (
        <StatefulNumberField
          ...
          onReset={
            canBeReset(ThSpacingSettingsKeys.paragraphIndent)
