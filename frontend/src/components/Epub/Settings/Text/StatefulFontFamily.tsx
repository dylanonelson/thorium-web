...
export const StatefulFontFamily = ({
  standalone = true,
  ...
  const updatePreference = useCallback(
    async (key: Key | null) => {
      ...
      const selectedOption = fontFamilyOptions.current.find(
      ...
      if (selectedOption) {
        ...
        const selectedOptionId = Object.keys(defaultFontFamilyOptions).find(
          (key) =>
            defaultFontFamilyOptions[
              key as keyof typeof defaultFontFamilyOptions
            ] === currentSetting,
        ) as keyof typeof defaultFontFamilyOptions;
        dispatch(
          setFontFamily(selectedOptionId || defaultFontFamilyOptions.publisher),
        );
      }
    },
            ...
            >
              {(item) => (
                <ListBoxItem
                  ...
                  style={
                    { fontFamily: item.value || undefined } as CSSProperties
                  }
                >
                  {item.label}
                </ListBoxItem>
