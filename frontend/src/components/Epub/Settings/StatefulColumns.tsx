...
export const StatefulColumns = () => {
  ...
  const columnCount =
    useAppSelector((state) => state.settings.columnCount) || "auto";
  ...
  const updateEffectiveValue = useCallback(
    (preference: string, setting: number | null) => {
      const derivedValue =
        preference === "auto" || setting === null ? "auto" : setting.toString();
  ...
  const debouncedUpdate = useCallback(() => {
    const update = () =>
      updateEffectiveValue(columnCount, getSetting("columnCount"));
    debounce(update, 50)();
