...
export const StatefulTheme = () => {
  ...
  const themeItems = useRef<(ThemeKeyType | "auto")[]>(
    themeArray.filter((theme: ThemeKeyType | "auto") => {
      if (theme === "auto") {
        return (
          preferences.theming.themes.systemThemes !== undefined &&
  ...
  const { onKeyDown } = useGridNavigation({
    ...
    onFocus: (id) => {
      const element = radioGroupWrapperRef.current?.querySelector(
        `[id="${id}"]`,
      );
      ...
      >
        <div
          ...
          className={classNames(
            settingsStyles.readerSettingsRadioWrapper,
            settingsStyles.readerSettingsThemesWrapper,
          )}
        >
          {themeItems.current.map((themeItem) => (
            ...
            >
              <span>
                {t(`reader.settings.themes.${themeItem}`, {
