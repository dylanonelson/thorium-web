...
export const StatefulTocContainer = ({
  triggerRef,
  ...
  const filterTocTree = (items: TocItem[], filterValue: string): TocItem[] => {
    ...
    return result.map((item: TocItem, index: number) => ({
      ...item,
      key: `${item.id}-${index}`,
    }));
  };
  ...
  useEffect(() => {
    if (
      actionState?.isOpen &&
      (!actionState?.docking ||
        actionState?.docking === ThDockingKeys.transient)
    ) {
      const handleEscape = (event: KeyboardEvent) => {
        if (
          !isActiveElement(searchInputRef.current) &&
          !filterValue &&
          event.key === "Escape"
        ) {
          setOpen(false);
                  ...
                  >
                    <TreeItemContent>
                      ...
                      <div className={tocStyles.tocTreeItemText}>
                        <div className={tocStyles.tocTreeItemTextTitle}>
                          {item.title}
                        </div>
                        {item.position && (
                          <div className={tocStyles.tocTreeItemTextPosition}>
                            {item.position}
                          </div>
                        )}
                      </div>
