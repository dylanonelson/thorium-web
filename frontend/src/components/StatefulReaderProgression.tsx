...
export const StatefulReaderProgression = ({
  ...
  const breakpointsMap = useMemo(() => {
    return makeBreakpointsMap<
      ThFormatPrefValue<ThProgressionFormat | ThProgressionFormat[]>
    >({
  ...
  const displayFormat = useMemo(() => {
    ...
    const hasPositions =
      !!unstableTimeline?.progression?.currentPositions?.length;
    const hasProgression =
      unstableTimeline?.progression?.relativeProgression !== undefined;

    if (Array.isArray(variants)) {
      return (
        getBestMatchingFormat(variants, hasPositions, hasProgression) ||
  ...
  useEffect(() => {
    if (
      displayFormat === ThProgressionFormat.none ||
      !unstableTimeline?.progression
    ) {
      setDisplayText("");
      return;
      ...
      case ThProgressionFormat.positionsLeft:
        if (positionsLeft !== undefined) {
          text = t("reader.app.progression.positionsLeft", {
            count: positionsLeft,
            reference:
              currentChapter || t("reader.app.progression.referenceFallback"),
          });
        }
        break;
