...
export const useDocking = <T extends string>(key: T) => {
  ...
  const dockablePref =
    preferences.actions.keys[key as keyof typeof preferences.actions.keys]
      ?.docked?.dockable || ThDockingTypes.none;

  const defaultSheet =
    preferences.actions.keys[key as keyof typeof preferences.actions.keys]
      ?.sheet?.defaultSheet || ThSheetTypes.popover;
  ...
  const sheetMap = makeBreakpointsMap<ThSheetTypes>({
    ...
    pref: preferences.actions.keys[key as keyof typeof preferences.actions.keys]
      ?.sheet?.breakpoints,
  });
  ...
  const canBeDocked = useCallback(
    (slot: ThDockingTypes.start | ThDockingTypes.end) => {
      return (
        (currentDockConfig === slot ||
          currentDockConfig === ThDockingTypes.both) &&
  ...
  const isDockedSheetPref = useCallback(
    (type?: ThSheetTypes.dockedStart | ThSheetTypes.dockedEnd) => {
      if (type) {
        return sheetPref === type;
      } else {
        return (
          sheetPref === ThSheetTypes.dockedStart ||
          sheetPref === ThSheetTypes.dockedEnd
        );
      }
    },
    [sheetPref],
  ...
  const getDocker = useCallback((): ThDockingKeys[] => {
    ...
    if (
      sheetPref === ThSheetTypes.fullscreen ||
      sheetPref === ThSheetTypes.bottomSheet
    )
      return [];
    ...
    if (dockerKeys.length === 1 && dockerKeys[0] === ThDockingKeys.transient)
      return [];

    return dockerKeys;
  }, [
    preferences.docking.displayOrder,
    currentDockConfig,
    sheetPref,
    dockablePref,
    canBeDocked,
  ...
  useEffect(() => {
    ...
    if (
      sheetType !== ThSheetTypes.dockedStart &&
      sheetType !== ThSheetTypes.dockedEnd
    ) {
      if (
        previousSheetType === ThSheetTypes.dockedStart ||
        previousSheetType === ThSheetTypes.dockedEnd
      ) {
        dispatch(
          setActionOpen({
            key: key,
