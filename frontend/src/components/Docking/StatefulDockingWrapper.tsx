...
const DockHandle = ({
  ...
  const classFromFlow = useCallback(() => {
    if (flow === ThDockingKeys.start) {
      return direction === ThLayoutDirection.ltr
        ? dockingStyles.dockResizeHandleGrabLeft
        : dockingStyles.dockResizeHandleGrabRight;
        ...
        {isResizable && hasDragIndicator && (
          <div
            className={classNames(
              dockingStyles.dockResizeHandleGrab,
              classFromFlow(),
            )}
          ></div>
        )}
...
const DockPanel = ({
  ...
  const dockClassName =
    flow === ThDockingKeys.end && direction === ThLayoutDirection.ltr
      ? "right-dock"
      : "left-dock";
    ...
    <>
      ...
      >
        <div
          ...
          className={classNames(
            dockingStyles.dockPanelContainer,
            dockClassName,
          )}
        ></div>
      </Panel>
...
export const StatefulDockingWrapper = ({
  ...
  const dockingStart = useAppSelector(
    (state) => state.actions.dock[ThDockingKeys.start],
  );
  const dockingEnd = useAppSelector(
    (state) => state.actions.dock[ThDockingKeys.end],
  );
  ...
  if (!preferences.docking.dock) {
    ...
    const dockConfig =
      (breakpoint && dockingMap[breakpoint]) || ThDockingTypes.both;

    return (
      <>
        <PanelGroup direction="horizontal">
          {(dockConfig === ThDockingTypes.both ||
            dockConfig === ThDockingTypes.start) && (
            <DockPanel
