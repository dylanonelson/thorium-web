...
export const StatefulBackLink = ({ className }: { className?: string }) => {
  const { t } = useI18n();
  ...
  const variant =
    preferences.theming.header?.backLink?.variant || ThBackLinkVariant.arrow;
  const href = preferences.theming.header?.backLink?.href;
  const content = preferences.theming.header?.backLink?.content;
  const visibility =
    preferences.theming.header?.backLink?.visibility || "partially";
  const backLinkClassName = classNames(
    backLinkStyles.backLink,
    visibility === "always"
      ? readerSharedUI.alwaysVisible
      : readerSharedUI.partiallyVisible,
  );
