import type { PublicationConfig, PublicationManifestMap } from "@/types/publications";

const manifestMap = Object.freeze(parseManifestEnv(process.env.NEXT_PUBLIC_PUBLICATION_MANIFESTS));

export const PUBLICATION_MANIFESTS: Readonly<PublicationManifestMap> = manifestMap;
...
function parseManifestEnv(rawValue?: string): PublicationManifestMap {
  ...
  try {
    ...
    return Object.entries(parsed).reduce<Record<string, PublicationConfig>>((acc, [key, value]) => {
      if (typeof key !== "string" || !key.trim() || !value || typeof value !== "object") {
        logManifestWarning("Invalid key or value found in publication catalog");
        return acc;
      }
      const entry = value as Record<string, unknown>;
      const manifestUrl = typeof entry.manifestUrl === "string" ? entry.manifestUrl : "";
